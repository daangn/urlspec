...
- **페이지 어노테이션**: `@key = value;` 로 페이지에 외부 스키마가 해석하는 메타데이터 부착
...
페이지 선언 위에 `@key = value;` 형태로 페이지에 대한 사실을 붙입니다.
...
**핵심 원칙**: URLSpec 은 줄의 **모양만** 검사합니다. 어떤 키가 유효한지, 값이 무엇을 뜻하는지는 스펙을 소비하는 쪽(자체 서버 스키마, 코드 생성기 등)이 정합니다. `@` 는 "여기부터 URLSpec 이 모른다"는 경계 표시입니다.

- **값 타입**: 문자열, `true`/`false`, 문자열 리스트 (`["a", "b"]`)
- **리스트**: 줄바꿈·트레일링 콤마 허용. 개수가 문법에서 읽혀야 하므로 대괄호 필수
- **붙는 대상**: `page` 선언만. `param`/`global` 위에 쓰면 파싱 에러
- **검증 규칙**: 키는 camelCase, 한 페이지에 같은 키 중복 금지 — 이 둘뿐
- **긴 문자열 문법 없음**: 배포와 함께 반영돼도 되는 짧은 사실만 담는다는 전제

**리졸브 결과**: `ResolvedPage.annotations` 는 `@` 를 뗀 `Record<string, string | boolean | string[]>`. 어노테이션이 없는 페이지는 필드 자체가 `undefined`.

**빌더**: `spec.addPage({ ..., annotations: { minAppVersion: "24.30.0" } })`

**프린터**: 설명 주석 → 어노테이션 → `page` 선언 순으로 정규화 출력. 리스트가 80칼럼을 넘으면 여러 줄로 나눕니다.
...
8. **어노테이션 키**: camelCase 만 허용 (`@minAppVersion`), 한 페이지에 같은 키 중복 금지
